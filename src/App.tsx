...
function App() {
  ...
  for(const collection of WINNING_COMBINATIONS){
    const firstSequence = gameBoard[collection[0].row][collection[0].column]
    const secondSequence = gameBoard[collection[1].row][collection[1].column]
    const thirdSequence = gameBoard[collection[2].row][collection[2].column]

    if(firstSequence && firstSequence === secondSequence && firstSequence === thirdSequence){
      setWinner(firstSequence)
    }
  ...
  return (
    <>
      ...
      <main id="game-container">
        ...
        {winner && <GameOver currentWinner={winner}/>}
      </main>
      <Log _turns = {turns}/>
